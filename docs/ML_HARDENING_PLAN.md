...
The app has useful UX, but its food-vision behavior is not yet product-grade. The checked-in CNN metadata records 4.0% accuracy and the artifact is not served. The current food scan rejects text-only estimates and requires a remote image candidate to agree with the user-confirmed dish name before returning a fixed food-factor estimate. The Food-101 archive has been audited against its official split and a reproducible transfer-learning script is ready, but no release-quality Food-101 training result exists yet. The annual-carbon benchmark now has an 18-field LightGBM product candidate that excludes the source dataset's sensitive `Sex` field; it is still a single random-holdout result, not release approval. The activity screen correctly uses a transparent projection because it does not infer uncollected annual-profile fields. The future planner uses a deterministic scenario calculator; it no longer generates pseudo-history.
...
- Generate metrics JSON files for every model that is eligible for a quality claim.
...
```text
python scripts/audit_food101_archive.py --archive "archive (8).zip"
python scripts/train_food101_transfer.py --archive "archive (8).zip" --epochs 8 --batch-size 64 --finetune-backbone
```

Run the full training command on a GPU machine. The archive contains 101,000 images; a CPU-only run is impractical for a release candidate. A capped `--max-*-per-class` invocation only checks the pipeline and must not be reported as model accuracy.
...
```text
complete 18-field user lifestyle schema -> preprocessing pipeline -> LightGBM candidate -> annual kg CO2e
```

Required changes:

- Do not collect the source dataset's `Sex` field merely to improve a holdout score.
...
- The UI must block the annual candidate when required profile fields are missing; it must not silently add defaults.
...
- Add prediction logging with explicit user consent.
- Store user corrections to food predictions without retaining uploaded images by default.
