...
The checked-in ResNet-18 metadata reports **4.0% accuracy** and the artifact is not used by the running food-scanner endpoint. The scan endpoint requires an optional external vision candidate (Hugging Face and/or Gemini) to agree with the user-confirmed dish name before it returns a food-factor estimate; text-only dish names are rejected. Provider scores are not calibrated and must not be treated as product accuracy.
