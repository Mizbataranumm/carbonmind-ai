...
import {
  ...
  Phone, Zap, Car, Utensils, Flame, Award, Target, BarChart2, CalendarDays, ShieldCheck, BrainCircuit
} from 'lucide-react';
import VoiceCallModal from '@/components/VoiceCallModal';
import LogActivityModal from '@/components/LogActivityModal';
import PhoneCallModal from '@/components/PhoneCallModal';
import { getCarbonIntelligence, getCarbonStats, predictWeekly } from '@/lib/api';
...
export default function Dashboard() {
  ...
  const refreshStats = useCallback(() => {
    ...
    getCarbonIntelligence(user.id).then(setIntelligence).catch(() => setIntelligence(null));
  }, [user?.id]);
  ...
  useEffect(() => {
    const history = stats?.consecutive_daily_history || [];
    if (history.length < 5) {
      setWeeklyForecast(null);
      return undefined;
    }
    let active = true;
    predictWeekly({ daily_history: history })
      .then((result) => active && setWeeklyForecast(result))
      .catch(() => active && setWeeklyForecast(null));
    return () => { active = false; };
  }, [stats]);
  ...
  const consecutiveHistory = stats?.consecutive_daily_history || [];
    ...
    <div className="w-full pb-8 space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {intelligence && (
        <section className="grid grid-cols-1 gap-4 lg:grid-cols-12">
          <div className="rounded-2xl border border-green/25 bg-widget p-5 shadow-sm lg:col-span-8">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="font-mono-data text-[11px] uppercase tracking-widest text-green">// Proactive carbon intelligence</div>
                <h2 className="mt-1 text-xl font-bold" style={{ color: 'var(--text-primary)' }}>{intelligence.next_action.title}</h2>
                <p className="mt-2 max-w-2xl text-sm leading-relaxed text-secondary">{intelligence.next_action.detail}</p>
              </div>
              <div className="flex min-w-28 flex-col items-center rounded-xl border border-glass-border bg-app/30 px-4 py-3">
                <BrainCircuit className="h-5 w-5 text-cyan" />
                <span className="mt-1 font-mono-data text-2xl text-green">{intelligence.readiness_score}</span>
                <span className="text-[10px] uppercase tracking-wide text-secondary">record readiness</span>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-1 gap-3 text-sm sm:grid-cols-3">
              <div className="rounded-xl border border-glass-border bg-app/20 p-3">
                <span className="text-xs text-secondary">Observed history</span>
                <p className="mt-1 font-mono-data text-green">{intelligence.readiness_components.consecutive_history_days} consecutive days</p>
              </div>
              <div className="rounded-xl border border-glass-border bg-app/20 p-3">
                <span className="text-xs text-secondary">Evidence today</span>
                <p className="mt-1 font-mono-data text-green">{intelligence.evidence.verified_count} confirmed / {intelligence.evidence.activity_count} saved</p>
              </div>
              <div className="rounded-xl border border-glass-border bg-app/20 p-3">
                <span className="text-xs text-secondary">Forecast status</span>
                <p className="mt-1 text-xs leading-relaxed text-secondary">{intelligence.forecast_readiness.message}</p>
              </div>
            </div>
          </div>
          <div className="rounded-2xl border border-glass-border bg-widget p-5 lg:col-span-4">
            <div className="flex items-center gap-2 text-green"><ShieldCheck className="h-5 w-5" /><span className="font-mono-data text-[11px] uppercase tracking-widest">Evidence policy</span></div>
            <p className="mt-3 text-sm leading-relaxed text-secondary">Manual entries remain labelled as user-entered. A confirmed food scan is recorded separately as photo-backed evidence. Missing days are never filled with zeroes.</p>
          </div>
        </section>
      )}
        ...
        <div className="lg:col-span-8 space-y-6 flex flex-col">
          ...
          <div className="bg-widget border border-glass-border rounded-3xl p-5 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="font-mono-data text-[11px] uppercase tracking-widest text-green">// Weekly outlook</div>
                <p className="mt-1 font-bold text-xl" style={{ color: 'var(--text-primary)' }}>Next seven days</p>
              </div>
              <div className="flex items-center gap-2 rounded-lg border border-glass-border bg-glass-bg px-3 py-1.5 text-xs text-secondary">
                <CalendarDays className="h-3.5 w-3.5 text-cyan" />
                {consecutiveHistory.length} consecutive saved days
              </div>
            </div>
            {weeklyForecast ? (
              <>
                <div className="mt-4 grid grid-cols-7 gap-2">
                  {weeklyForecast.forecast_days.map((day, index) => (
                    <div key={day} className="min-w-0 rounded-xl border border-glass-border bg-app/30 p-2 text-center">
                      <div className="text-[10px] text-secondary">{day}</div>
                      <div className="mt-1 font-mono-data text-sm text-green">{weeklyForecast.forecast[index]}</div>
                      <div className="text-[9px] text-secondary">kg</div>
                    </div>
                  ))}
                </div>
                <p className="mt-3 text-xs leading-relaxed text-secondary">
                  {weeklyForecast.method === 'holt_winters_only_lstm_not_validated_or_insufficient_history'
                    ? 'Holt-Winters forecast from your observed activity history. The LSTM is not used until a validated dated-data model exists.'
                    : `Forecast method: ${weeklyForecast.method}.`}
                </p>
              </>
            ) : (
              <div className="mt-4 rounded-xl border border-dashed border-glass-border p-4 text-sm leading-relaxed text-secondary">
                Save activities on five consecutive days to unlock a forecast based on your own record. Missing days are not treated as zero-emission days.
              </div>
            )}
          </div>
        </div>
