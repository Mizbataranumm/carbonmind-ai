    ...
    <div className="space-y-6" data-testid="tracker-root">
      ...
      <div className="glass p-6 glass-hover">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <div className="font-mono-data text-[10px] uppercase tracking-widest text-[#00FFB2]">// Live telemetry · 24h</div>
            <div className="font-display text-2xl mt-1">Real-time emissions</div>
            <p className="text-sm text-[#9EABBC] mt-1">Granular per-activity carbon attribution.</p>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#00FFB2]/10 border border-[#00FFB2]/25">
            <span className="h-2 w-2 rounded-full bg-[#00FFB2] animate-pulse" style={{ boxShadow: "0 0 10px #00FFB2" }} />
            <span className="font-mono-data text-xs text-[#00FFB2]">streaming live</span>
          </div>
        </div>
        <div className="h-[230px] mt-5">
          <ResponsiveContainer>
            <AreaChart data={data.realtime} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="kg" stroke="#00FFB2" strokeWidth={2.5} fill="url(#rtg)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
        ...
        {data.categories.map((c, i) => (
          ...
          >
            <div className="flex items-center justify-between">
              <div className="font-mono-data text-[10px] uppercase tracking-widest text-[#9EABBC]">{c.name}</div>
              <span className="h-2 w-2 rounded-full" style={{ background: c.color, boxShadow: `0 0 8px ${c.color}` }} />
            </div>
            <div className="font-mono-data text-3xl mt-2" style={{ color: c.color }}>{c.kg}<span className="text-sm text-[#9EABBC] ml-1">kg</span></div>
            <div className="text-xs text-[#9EABBC] mt-1">Budget {c.budget} kg</div>
            ...
            <div className={`flex items-center gap-1 text-xs font-mono-data mt-3 ${c.trend <= 0 ? "text-[#00FFB2]" : "text-[#FFD166]"}`}>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <div className="glass p-6 glass-hover lg:col-span-2" data-testid="activity-feed">
          <div className="flex items-center justify-between mb-4">
            <div>
              <div className="font-mono-data text-[10px] uppercase tracking-widest text-[#00FFB2]">// Activity feed</div>
            ...
            {data.activities.map((a) => {
              ...
              return (
                <motion.div
                  key={a.id}
                  whileHover={{ x: 4 }}
                  className="flex items-center gap-4 p-3.5 rounded-xl bg-white/[0.02] border border-white/[0.04] hover:border-[#00FFB2]/20 transition"
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="font-mono-data text-[11px] text-[#9EABBC]">{a.time} · {a.type}</div>
                  </div>
                  <div className="font-mono-data text-base text-white">{a.kg.toFixed(1)} kg</div>
                </motion.div>
              );
            })}
          </div>
        ...
        <div className="glass p-6 glass-hover" data-testid="heatmap-card">
          <div className="font-mono-data text-[10px] uppercase tracking-widest text-[#00FFB2]">// 8-week heatmap</div>
          ...
          <div className="flex items-center justify-between mt-4 text-[10px] font-mono-data text-[#9EABBC]">
            <span>Less</span>
