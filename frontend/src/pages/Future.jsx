...
import { ArrowRight, Car, Leaf, Plane, PlugZap, RotateCcw, Sparkles, Trees } from "lucide-react";
...
const defaultForm = { transport: "mixed", diet: "mixed", electricity_kwh: 3200, flights_per_year: 2, horizon_years: 10 };

const presets = [
  { id: "balanced", label: "Balanced routine", values: defaultForm },
  { id: "lower-impact", label: "Lower-impact shift", values: { transport: "public", diet: "vegetarian", electricity_kwh: 2400, flights_per_year: 1, horizon_years: 10 } },
  { id: "travel", label: "Frequent travel", values: { transport: "car", diet: "mixed", electricity_kwh: 4200, flights_per_year: 5, horizon_years: 10 } },
];
...
export default function Future() {
  ...
  const signals = [
    { icon: Car, label: "Mobility", value: transportOptions.find((option) => option.id === form.transport)?.label },
    { icon: PlugZap, label: "Electricity", value: `${form.electricity_kwh.toLocaleString()} kWh / year` },
    { icon: Plane, label: "Flights", value: `${form.flights_per_year} / year` },
  ];
      ...
      <section className="glass overflow-hidden p-5 sm:p-7">
        <div className="grid items-end gap-6 lg:grid-cols-[1fr_auto]">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-secondary">Explore how a chosen lifestyle pattern changes a transparent annual-emissions scenario over time. It is a planning calculator, not an LSTM forecast or a prediction of the climate.</p>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        <form onSubmit={submit} className="glass space-y-6 p-5 sm:p-6 lg:col-span-2" data-testid="future-form">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="future-electricity" className="font-mono-data text-[10px] uppercase tracking-widest text-secondary">Electricity / year</label>
              <input id="future-electricity" data-testid="input-electricity" type="number" min="0" value={form.electricity_kwh} onChange={(event) => update("electricity_kwh", Number(event.target.value) || 0)} className="input-glass mt-2" />
              <p className="mt-1 text-xs text-secondary">kWh</p>
            </div>
            <div>
              <label htmlFor="future-flights" className="font-mono-data text-[10px] uppercase tracking-widest text-secondary">Flights / year</label>
              <input id="future-flights" data-testid="input-flights" type="number" min="0" max="100" value={form.flights_per_year} onChange={(event) => update("flights_per_year", Number(event.target.value) || 0)} className="input-glass mt-2" />
              <p className="mt-1 text-xs text-secondary">one-way estimate</p>
            </div>
          </div>
          ...
          {!result ? (
            <div className="glass flex min-h-[490px] flex-col justify-between p-5 sm:p-7" data-testid="future-empty">
              ...
              <div className="my-8 grid gap-3 sm:grid-cols-2">
                ...
                <InputSummary label="Electricity" value={`${form.electricity_kwh.toLocaleString()} kWh/year`} />
                <InputSummary label="Flights" value={`${form.flights_per_year} per year`} />
              </div>
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-5" data-testid="future-result">
              ...
              <div className="glass min-w-0 p-5 sm:p-6">
                ...
                <div className="mt-4 h-[230px]"><ResponsiveContainer width="100%" height="100%"><LineChart data={result.yearly_breakdown} margin={{ top: 10, right: 15, left: -15, bottom: 0 }}><CartesianGrid stroke="rgba(255,255,255,0.04)" strokeDasharray="3 3" /><XAxis dataKey="year" stroke="#5C6B7A" fontSize={11} tickLine={false} axisLine={false} /><YAxis stroke="#5C6B7A" fontSize={11} tickLine={false} axisLine={false} /><Tooltip contentStyle={{ background: "var(--bg-accent)", border: "1px solid var(--glass-border)", borderRadius: 8, color: "var(--text-primary)" }} /><Line type="monotone" dataKey="co2" stroke="var(--neon-green)" strokeWidth={2.5} dot={{ r: 3, fill: "var(--neon-green)" }} /></LineChart></ResponsiveContainer></div>
              </div>
